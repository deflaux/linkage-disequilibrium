...
public class LdVariantTest {
  ...
  private void testExpectedLdAndReverseAndNeg(double dPrime, double r, Genotype[] a, Genotype[] b) {
    ...
    Assert.assertArrayEquals(new double[] {dPrime, r},
        getDPrimeAndR(flipGenotypes(b), flipGenotypes(a)), 1e-6);
  ...
  public void testComputeLd() {
    ...
    testExpectedLdAndReverseAndNeg(1.0, 1.0, new Genotype[] {ZERO, ONE},
        new Genotype[] {ZERO, ONE});

    testExpectedLdAndReverseAndNeg(1.0, 1.0, new Genotype[] {ONE, ZERO, ONE},
        new Genotype[] {ONE, ZERO, ONE});

    testExpectedLdAndReverseAndNeg(1.0, 1.0, new Genotype[] {ONE, ZERO, ONE, ONE},
        new Genotype[] {ONE, ZERO, ONE, ONE});

    testExpectedLdAndReverseAndNeg(1.0, 1.0, new Genotype[] {ONE, ZERO, UNK, ONE},
        new Genotype[] {ONE, ZERO, UNK, ONE});

    // Unknown positions must be ignored.
    testExpectedLdAndReverseAndNeg(1.0, 1.0, new Genotype[] {UNK, ZERO, ONE, ONE},
        new Genotype[] {ONE, ZERO, UNK, ONE});

    testExpectedLdAndReverseAndNeg(1, 0.5773503, new Genotype[] {ONE, ONE, ONE, ZERO},
        new Genotype[] {ONE, ONE, ZERO, ZERO});

    testExpectedLdAndReverseAndNeg(1.0, 0.66666667, new Genotype[] {ONE, ONE, ONE, ZERO, ZERO},
    ...
    testExpectedLdAndReverseAndNeg(1.0, 0.66666667, new Genotype[] {ONE, ONE, ONE, ZERO, ZERO, UNK},
        new Genotype[] {ONE, ONE, ZERO, ZERO, ZERO, ONE});
  }
}
