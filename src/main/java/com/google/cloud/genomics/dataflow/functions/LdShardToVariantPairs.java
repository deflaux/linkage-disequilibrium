...
/**
 * Part of the LinkageDisequilibrium pipeline that consumes pairs of LdVariant lists (one query and
 * one target), and emits all pairs of LdVariants to cover all pairs of comparisons where the
 * LdVariant from the query is "before" (as defined by LdVariant.compareTo()) the target LdVariant
 * and the end of query is within window of the start of target.
 */
public class LdShardToVariantPairs
   ...
   *
   * All the pairs that are emitted here must be desired comparisons with no duplicates -- no
   * additional filters are done other than (optionally) on the LD value itself.
   */
  @Override
